 ...
 * @author 36194445
 */
public class OfertaBaseCriteriaPostgre implements EstablecimientosCriteriaPostgre{
                    ...
                    List<Localizacion> loct = e.getLocalizacion().stream()
                        .filter(l->{
                            List<OfertaLocal> ofertas = l.getOfertas().stream()
                                .filter(oferta -> oferta.getOferta()!=null&&oferta.getOferta().getBase()!=null&&list.contains(oferta.getOferta().getBase().getId().intValue()))
